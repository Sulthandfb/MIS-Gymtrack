...
import {
  ...
  Download,
  RefreshCw,
  // Tambahan ikon jika diperlukan
  Meh, // for Neutral sentiment
  Laugh, // for Positive topic
  Frown, // for Negative topic
  Users, // for member in feedback list
  Calendar, // for date in feedback list
  Tag, // for feedback type in feedback list
} from "lucide-react";
import {
  LineChart,
  Line,
  ...
  BarChart, Bar, // ✅ FIXED: Re-added BarChart and Bar
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical">
            ...
            <Tooltip
              formatter={(value: any, name: string) => [`${value} (${(data.find(d => d.sentiment === name)?.percentage || 0).toFixed(1)}%)`, "Jumlah Feedback"]}
            />
...
const FeedbackList = ({ feedback, feedbackTypes, memberNames, onFilterChange }: { 
    ...
    <div className="bg-white p-4 lg:p-6 rounded-xl border border-gray-200 shadow-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
            <Select onValueChange={(value) => setCurrentFilters({...currentFilters, sentiment: value === "all" ? undefined : value})}>
            ...
            <Select onValueChange={(value) => setCurrentFilters({...currentFilters, feedback_type: value === "all" ? undefined : value})}>
...
export default function SentimentDashboard() {
  ...
  const loadDashboardData = async (filters?: FeedbackFilters, trendTimeRange: string = "all-time") => {
    setLoading(true);
    setError(null);
    try {
  ...
  if (!dashboardData) { // Fallback if dashboardData is null after loading (shouldn't happen with error handling)
    return <div className="text-center py-10">No data available.</div>;
  }

  const { summary, sentimentDistribution, topicAnalysis, sentimentTrendDaily, recentFeedback, allAIInsights, feedbackTypes, memberNames } = dashboardData;
        ...
        <header className="bg-white border-b border-gray-200 px-4 lg:px-6 py-4 flex items-center justify-between sticky top-0 z-10">
          <div>
            ...
            <p className="text-gray-600 text-xs lg:text-sm">
              Analisis mendalam dari AI tentang sentimen dan topik
            </p>
          </div>
          <div className="flex gap-2">
            <Button
            ...
            <Button className="flex items-center gap-2 px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50" variant="outline">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={sentimentTrendDaily}>
                      ...
                      <Tooltip
                        ...
                        formatter={(value: any, name: string) => [value, name === "positive" ? "Positif" : name === "neutral" ? "Netral" : "Negatif"]}
                      />
